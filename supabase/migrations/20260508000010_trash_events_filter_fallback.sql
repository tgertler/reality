-- Add fallback for trash-event feed selection when event_entity_type is null.
...
BEGIN
  RETURN (
    WITH upcoming AS (
      ...
      WHERE ce.trash_event_id IS NOT NULL
        AND (ce.event_entity_type = 'trash_event' OR ce.event_entity_type IS NULL)
        AND ce.start_datetime >= now()
      ORDER BY te.id, ce.start_datetime ASC
    ),
    ranked AS (
      SELECT *
      FROM upcoming
